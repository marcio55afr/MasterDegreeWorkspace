...
class SearchTechniqueCV(BaseClassifier):
    ...
    def fit(self, data, labels):
        ...
        self.windows = ResolutionMatrix(self.ts_length,
                                        self.word_length,
                                        self.num_windows,
                                        self.max_window_length).matrix.columns.values
        ...
        for window in self.windows:
            if self.verbose:
                print('#', end='')
            disc = SFA(word_length=self.word_length,
        ...
        if self.select_features:
            ...
            bag_of_words = self._feature_selection(bag_of_words)
    ...
    def _get_feature_matrix(self, word_sequence):
        ...
        vectorizer = DictVectorizer(sparse=True,
                                    dtype=np.int32,
                                    sort=False)
        bag_of_words = vectorizer.fit_transform(word_sequence[0])
        bag_of_words = pd.DataFrame(bag_of_words.todense(),
                               index=word_sequence.index,
