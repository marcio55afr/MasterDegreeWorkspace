...
class Evaluator:
    ...
    def fit_runtime(self, unit="s", train_or_test="test", cv_fold="all"):
        ...
        # change column dtypes to allow operations (+ , - , ...)
        run_times["fit_estimator_start_time"] = [pd.Timestamp(s) for s in run_times["fit_estimator_start_time"]]
        run_times["fit_estimator_end_time"] = [pd.Timestamp(s) for s in run_times["fit_estimator_end_time"]]
        run_times["predict_estimator_start_time"] = [pd.Timestamp(s) for s in run_times["predict_estimator_start_time"]]
        run_times["predict_estimator_end_time"] = [pd.Timestamp(s) for s in run_times["predict_estimator_end_time"]]
