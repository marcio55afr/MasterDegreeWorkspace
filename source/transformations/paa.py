...
class PAA(_PanelToPanelTransformer):
    ...
    def transform_univariate(self, X, y=None):
        """
        Overview: In order to speed up some proccessing this function
        receives a set of time series of only one dimension
            for each time_series:
                approximated_ts = paa(time_series)
                appr_set.append(approximated_ts)
            return ts_appr_set

        Parameters
        ----------
        X : pandas DataFrame or 2d-array of shape [n_instances, ts_length]
            Dataframe with univariate time series in each row.

        Returns
        -------
        appr_set: Numpy 2d-array data frame with first dimension in column zero,
              second in column one etc.
        """
