...
class AdaptedSFA(_PanelToPanelTransformer):
    ...
    Transformer, 
    authored by Matthew Middlehurst and Patrick Schäfer and described in
    ...
    create ngram words of any size and then count the frequencies.
    
    The parameters save_words, bigrams and skip_grams were removed. The first
    to save some memory and the others to avoid reworking, since this version
    were created to build ngrams after the transformation.
    ...
    def __init__(
        ...
        super(AdaptedSFA, self).__init__()
    ...
    def _transform_case(self, X, i, supplied_dft):
        ...
        for window in range(dfts.shape[0]):
            word_raw = AdaptedSFA._create_word(
                dfts[window], self.word_length, self.alphabet_size, self.breakpoints
            )

            repeat_word = (
                self._add_to_pyramid(
                    word_sequence, word_raw, last_word, window - int(repeat_words / 2)
                )
                if self.levels > 1
                else self._add_to_sequence(word_sequence, word_raw, last_word, window)
            )
        ...
        return pd.Series(word_sequence) if self.return_pandas_data_series else word_sequence
    ...
    def _mft(self, series, transformed=None, stds=None):
        ...
        stds = AdaptedSFA._calc_incremental_mean_std(series, end, self.window_size, stds)
    ...
    def _add_to_sequence(self, word_sequence, word, last_word, offset):
